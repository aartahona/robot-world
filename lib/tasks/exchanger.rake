...
namespace :exchanger do
    ...
    task verify_exchanges: [:environment] do
        ...
        unless pending_exchanges.empty?
            pending_exchanges.each do |exchange|
                ...
                new_model = CarModel.find_by(name: exchange.wanted_model)

                if new_model.nil?
                    $exchanger_logger.error ("The car model: #{exchange.wanted_model} doesn't exists")
    ...
    def exchange_order(order, new_model_id)
        ...
        $exchanger_logger.info ("Car Replaced: Car_id: #{replaced_car.id} | Model: #{replaced_car.car_model.name} | Year: #{replaced_car.car_model.year}")
